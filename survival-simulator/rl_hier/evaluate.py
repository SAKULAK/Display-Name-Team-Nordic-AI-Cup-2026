...
def evaluate_episode(policy, config, seed, horizon, render=False, deadline=float("inf"),
                     policy_mode="deterministic", policy_seed=20260918):
...
def validate(policy, config, horizon, deadline=float("inf"), should_stop=lambda: False):
    ...
    from rl.execution import PolicyExecution
    from rl_hier.validation import preserve_training_rng, validation_plan
    episodes = []
...
def parse_args(argv=None):
    ...
    parser.add_argument("--output", type=Path, default=Path("rl_hier/runs/evaluation.csv"))
    parser.add_argument("--policy-mode", choices=("deterministic", "stochastic"), default="deterministic")
    parser.add_argument("--policy-seed", type=int, default=20260918)
    ...
    if args.policy_mode == "deterministic" and args.policy_rollouts != 1:
        parser.error("Deterministic evaluation requires one rollout per seed")
...
def aggregate(episodes):
    ...
    return dict(completed_episodes=len(complete), mean_survival=statistics.mean(survival),
                ...
                completion_rate=statistics.mean(e["horizon_reached"] for e in complete))


def main(argv=None):
    args = parse_args(argv)
    import torch
    from rl_hier.config import Config
    from rl_hier.checkpoint import load_checkpoint
    from rl_hier.policy import SharedPolicy, choose_device
    from rl_hier.metrics import CSVLogger, validation_rank
    ...
    episodes = []
    total = len(seeds) * args.policy_rollouts
    if args.policy_mode == "stochastic" and args.policy_seed + total > 2**63:
        raise ValueError("Derived evaluation policy seeds exceed 2**63-1")
    plan = [(seed, index, args.policy_seed + env_index * args.policy_rollouts + index)
            for env_index, seed in enumerate(seeds) for index in range(args.policy_rollouts)]
    for seed, rollout_index, policy_seed in plan:
        ...
        episode.update(environment_seed=seed, rollout_index=rollout_index)
        log.write(episode)
